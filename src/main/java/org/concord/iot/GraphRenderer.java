...
class GraphRenderer {
    ...
    final static String[] DATA_TYPES = new String[]{"Temperature (" + '\u2103' + ")", "Barometric pressure (hPa)", "Relative humidity (%)", "Visible light (lux)", "Infrared light (lux)", "Distance (mm)", "Acceleration (?)"};
    ...
    void drawFrame(Graphics2D g) {
        ...
        double dyMultiplied = (ymax - ymin) / nTickmarksOfYAxis;
        while (dyMultiplied < 1) {
            dyMultiplied *= 10;
            tens *= 10;
        }
        dyMultiplied = Math.ceil(dyMultiplied);
        double yminMultiplied = Math.floor(ymin * tens);
        double scaleY = h / (ymax - ymin);
        for (int i = 0; i < nTickmarksOfYAxis + 1; i++) {
            double yTickmark = (yminMultiplied + i * dyMultiplied) / tens;
    ...
    void drawData(Graphics2D g, List<SensorDataPoint> data, String label, boolean highlight) {
        ...
        if (n > 0) {
            ...
            SensorDataPoint d = data.get(0);
            double t1 = d.getTime();
            double v1 = d.getValue();
            double t2, v2;
            int x1 = 0, y1 = 0, x2 = 0, y2 = 0;
            double scaleX = w / xmax;
            double scaleY = h / (ymax - ymin);
            synchronized (data) {
    ...
    double[] getData(List<SensorDataPoint> data, int rx, int ry) {
        if (!data.isEmpty()) {
            double t, v;
            int dx = 0, dy = 0;
            double scaleX = w / xmax;
